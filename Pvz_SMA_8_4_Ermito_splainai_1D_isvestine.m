...
for j=1:2
    ...
    [DU,DV]=D_Hermite(X(iii:iii+1),j,xxx);
    fff=fff+U*F(iii+j-1)+V*DF(iii+j-1);
    dfff=dfff+DU*F(iii+j-1)+DV*DF(iii+j-1);
end
...
plot(X(iii:iii+1),DF(iii:iii+1),'ko','LineWidth',2,'MarkerSize',8)
end %-----------------ciklas per intervalus pabaiga
legend({'duotoji funkcija sin(x)',sprintf('Ermito splainai %d intervaluose: f',nP-1),...,
    'duotosios funkcijos isvestine',sprintf('Ermito splainai %d intervaluose: df/dx',nP-1)});
return
end
...
function [DU,DV]=D_Hermite(X,j,x)  % Ermito daugianariu isvestines
    L=Lagrange(X,j,x); 
